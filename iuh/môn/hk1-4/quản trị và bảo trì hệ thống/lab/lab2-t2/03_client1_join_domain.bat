@echo off
title TU DONG CAU HINH VA JOIN DOMAIN NEWSTAR.VN (CLIENT 1)
color 1f

echo =====================================================================
echo    TU DONG CAU HINH DNS, DOI TEN VA JOIN DOMAIN CHO CLIENT WIN 7 (1)
echo =====================================================================
echo.

:: 1. Cau hinh DNS tro ve Domain Controller
echo [1/3] Dang tro DNS ve Domain Controller (192.168.1.132 / 192.168.11.1)...
powershell -Command "Get-WmiObject Win32_NetworkAdapterConfiguration -Filter 'IPEnabled=True' | ForEach-Object { `$_.SetDNSServerSearchOrder(@('192.168.1.132', '192.168.11.1')) }"
...
if %errorlevel% neq 0 (
    echo [-] Khong phan giai duoc newstar.vn, dang kiem tra IP truc tiep 192.168.11.1...
    ping 192.168.11.1 -n 2
) else (
...
echo.
echo [3/3] Dang tien hanh gia nhap Domain newstar.vn voi ten WIN7-PC1...

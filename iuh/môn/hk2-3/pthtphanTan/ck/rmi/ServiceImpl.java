...
public class ServiceImpl extends UnicastRemoteObject implements IService {
    ...
    // ====================================================================
    // KHI THI: UNCOMMENT BAI CAN LAM, COMMENT CAC BAI KHAC
    // Mac dinh: BAI 10 (Dao chuoi) dang UNCOMMENT
    // ====================================================================
    @Override
    public String xuLyDuLieu(String data) throws RemoteException {
        try {
            ...
            // long num = Long.parseLong(data.trim());
            // if (num < 2) return "Khong phai so nguyen to";
            // for (long i = 2; i * i <= num; i++) if (num % i == 0) return "Khong phai so nguyen to";
            ...
            // List<Double> list = new ArrayList<>();
            // for (String s : a) if (!s.isBlank()) list.add(Double.parseDouble(s.trim()));
            // list.sort((xx, yy) -> Double.compare(yy, xx));
            // return list.toString();

            // ========== BAI 8: Thong ke tu - Input: "phat trien he thong phat trien" ==========
            // String[] w = data.toLowerCase().trim().split("\\s+");
            // Map<String, Integer> map = new LinkedHashMap<>();
            // for (String word : w) if (!word.isBlank()) map.put(word, map.getOrDefault(word, 0) + 1);
            // return map.toString();
            ...
            // ========== BAI 10: Dao chuoi (MAC DINH DANG MO) ==========
            ...
            // long num = Long.parseLong(data.trim());
            // if (num < 0) return "n phai >= 0";
            // return String.valueOf(num * (num + 1) / 2);
